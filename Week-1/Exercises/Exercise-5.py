...
def prod_list_rec_full(lst):
    ...
    """
    non_numeric = [num for num in lst if not isinstance(num, (int,float))]
    if non_numeric : 
        print("non numerical values in the list")
        return
    if lst==[]:
        return 1
    else:
        return(lst[0]*prod_list_rec(lst[1:]))
    pass
print(prod_list_rec_full([2, 3, 4, 'a'])) 
sprint("---")
