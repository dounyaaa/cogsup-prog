...
gap=300
# I still have an impression of causality with a gap at 300 because it happens at the same time

for x in range(-400, -51 - gap, 5): #move the red square to the left by 5 pixels, every 1ms until it reaches position of the green square
    square_red.position =(x, 0)
    square_red.present(clear = True, update = False)
    square_green.present(clear=False, update = True)
    exp.clock.wait(1)

for x in range(50, 400, 5): #Move the green square to the right on the same distance and same speed as before
